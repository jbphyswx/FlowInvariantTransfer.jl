...
#
#   julia --project=benchmark benchmark/benchmarks.jl            # BenchmarkTools.run all
...
for axis in ("nonlinear_term", "spectral_flux", "shell_to_shell", "mode_to_mode",
    ...
    SUITE[axis] = BenchmarkTools.BenchmarkGroup()
end

for N in SIZES
    û, ks = field2d(N)
    b  = FIT.Types.LinearBinning(2π / (2π))
    ...
    if N <= 32
        SUITE["nonlinear_term"]["N$N/directsum"] =
            BenchmarkTools.@benchmarkable FIT.NonlinearTerm.compute_nonlinear_term!($ws, $û, $ks; spectral = FIT.SpectralBackends.DirectSumSpectralBackend()) evals=1
    end
    SUITE["nonlinear_term"]["N$N/fft"] =
        BenchmarkTools.@benchmarkable FIT.NonlinearTerm.compute_nonlinear_term!($ws, $û, $ks; spectral = FIT.SpectralBackends.FFTSpectralBackend()) evals=1

    # Spectral flux Π(K): serial vs threaded (FFT spectral).
    SUITE["spectral_flux"]["N$N/serial"] =
        BenchmarkTools.@benchmarkable FIT.SpectralFlux.calculate_spectral_flux($û, $ks; binning = $b,
            spectral = FIT.SpectralBackends.FFTSpectralBackend(), execution = FIT.ComputationalBackends.SerialBackend()) evals=1
    if THREADED
        SUITE["spectral_flux"]["N$N/threaded"] =
            BenchmarkTools.@benchmarkable FIT.SpectralFlux.calculate_spectral_flux($û, $ks; binning = $b,
                spectral = FIT.SpectralBackends.FFTSpectralBackend(), execution = FIT.ComputationalBackends.ThreadedBackend()) evals=1
    end

    # Shell-to-shell T(n,m): serial vs threaded (FFT spectral).
    SUITE["shell_to_shell"]["N$N/serial"] =
        BenchmarkTools.@benchmarkable FIT.ShellToShellTransfer.calculate_shell_to_shell_transfer($û, $ks; binning = $b,
            spectral = FIT.SpectralBackends.FFTSpectralBackend(), execution = FIT.ComputationalBackends.SerialBackend(), verify_antisymmetry = false) evals=1
    if THREADED
        SUITE["shell_to_shell"]["N$N/threaded"] =
            BenchmarkTools.@benchmarkable FIT.ShellToShellTransfer.calculate_shell_to_shell_transfer($û, $ks; binning = $b,
                spectral = FIT.SpectralBackends.FFTSpectralBackend(), execution = FIT.ComputationalBackends.ThreadedBackend(), verify_antisymmetry = false) evals=1
    end

    # Smooth band-to-band T(K,Q): serial vs threaded (one nonlinear term per band, FFT spectral).
    bands = FIT.Types.SmoothBands([2.0, 4.0, 8.0])
    SUITE["band_to_band"]["N$N/serial"] =
        BenchmarkTools.@benchmarkable FIT.BandTransfer.calculate_band_to_band_transfer($û, $ks; bands = $bands,
            spectral = FIT.SpectralBackends.FFTSpectralBackend(), execution = FIT.ComputationalBackends.SerialBackend()) evals=1
    if THREADED
    ...
    SUITE["compressible"]["N$N/serial"] =
        BenchmarkTools.@benchmarkable FIT.Compressible.calculate_compressible_flux($û, $ρ̂, $ks; binning = $b,
            spectral = FIT.SpectralBackends.FFTSpectralBackend(), execution = FIT.ComputationalBackends.SerialBackend()) evals=1
    if THREADED
    ...
    if N == first(SIZES)
        SUITE["mode_to_mode"]["N$N/serial"] =
            BenchmarkTools.@benchmarkable FIT.calculate_mode_to_mode_transfer($û, $ks;
                spectral = FIT.SpectralBackends.FFTSpectralBackend(), execution = FIT.ComputationalBackends.SerialBackend(), force = true) evals=1
        if THREADED
            SUITE["mode_to_mode"]["N$N/threaded"] =
                BenchmarkTools.@benchmarkable FIT.calculate_mode_to_mode_transfer($û, $ks;
                    spectral = FIT.SpectralBackends.FFTSpectralBackend(), execution = FIT.ComputationalBackends.ThreadedBackend(), force = true) evals=1
        end
    end
end
...
if abspath(PROGRAM_FILE) == @__FILE__
    ...
    println("\n== BenchmarkTools.median time / allocations ==")
